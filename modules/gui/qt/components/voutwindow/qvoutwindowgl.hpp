...
class QVoutWindowGL : public QVoutWindow
{
    Q_OBJECT
public:
    ...
    bool setupVoutWindow(vout_window_t* window) override;
...
private:
    ...
    std::unique_ptr<VideoSurfaceGL> m_surfaceProvider;
};

class VideoSurfaceGL : public VideoSurfaceProvider
{
public:
    VideoSurfaceGL(QVoutWindowGL* renderer, QObject* parent = nullptr);

private:
    QSGNode *updatePaintNode(QQuickItem* item, QSGNode *, QQuickItem::UpdatePaintNodeData *) override;

private:
    QVoutWindowGL* m_renderer = nullptr;
